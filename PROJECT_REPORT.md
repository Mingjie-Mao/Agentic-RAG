...
1. **固定 workflow 的优势不是自建题目的产物。** 在外部语料上它在每一项可比指标上都优于单次
   RAG（答案 62/150 对 57/150，gold 文档召回 0.578 对 0.518，引用召回 0.404 对 0.343，
   错误拒答更少），代价是延迟与 token。这是"默认路由走固定 workflow"这个决定的第一份外部证据。
