...
- **外部验证做了两批**，每批 150 题、只跑一次、互不重叠，609 篇英文新闻全部入库为干扰文档。第 1 批（修复前）固定 workflow 62/150、单次 RAG 57/150；第 2 批（两处确定性修复之后）两条 arm 都是 **101/150**，gold 文档召回 0.578 → 0.667。
...
- Yes-bias 已经做过机械归因：37 次 Yes/No 判错中 **24 次证据是齐的**，所以偏置主要不在检索；判对的 41 次里 34 次金标是 yes，在 47 yes / 30 no 的批次上与「倾向答 yes 恰好蒙对」难以区分。其余四类留给人工判读，归因之前不改 prompt。
...
- 下一步是 **P0：人工语义金标**——已产出 103 题 / 176 条 claim 的盲标标注集（entailment、relevance、completeness，外加标注者自己读出的结论），标签回来之后才校准评分器、才冻结、才跑第 3 批。不直接接一个 LLM judge 报「语义正确率」，那只是把不知道答案对不对换成不知道 judge 对不对。
